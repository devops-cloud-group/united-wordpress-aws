build:
	cd VPC && terraform workspace new prod 	|| terraform workspace select prod  && terraform init -reconfigure || terraform init &&  terraform  apply   -var-file ../prod.tfvars --auto-approve
	cd RDS && terraform workspace new prod 	|| terraform workspace select prod  && terraform init -reconfigure || terraform init &&  terraform  apply   -var-file ../prod.tfvars --auto-approve
	cd ASG && terraform workspace new prod 	|| terraform workspace select prod  && terraform init -reconfigure || terraform init &&  terraform  apply   -var-file ../prod.tfvars --auto-approve
